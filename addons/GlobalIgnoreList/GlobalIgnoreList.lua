...
local function hasIgnored (name)
	...
	for count = 1, GetNumIgnores() do
		...
		if name == Proper(addServer(GetIgnoreName(count))) then
			result = count
...
local function isValidList()
	...
	if GetNumIgnores() > 0 then
		...
		for count = 1, GetNumIgnores() do
			str = removeServer(GetIgnoreName(count), true)
...
function SyncIgnoreList (silent)
		...
		while listCount <= #GlobalIgnoreDB.ignoreList do
			...
			if GlobalIgnoreDB.typeList[listCount] == "player" then
				...
				if tries >= maxSyncTries then
					DelIgnore(removeServer(GlobalIgnoreDB.ignoreList[listCount]))
	...
	local ignoreCount = GetNumIgnores()
...
local function ApplicationStartup(self)
	...
	filterDefFilter[#filterDefFilter + 1] = "([contains=www] or [contains=w.w.w]) and ([contains=.c0m] or [contains=.c.0.m] or [contains=,com])"
	filterDefActive[#filterDefActive + 1] = true
	...
	filterDefFilter[#filterDefFilter + 1] = "(([contains=<] and [contains=>]) or ([contains=\\[] and [contains=\\]]) or ([contains=\\(] and [contains=\\)])) and ([contains=recruit] or [contains=progress] or [contains=raid] or [contains=guild])"
	...
	filterDefFilter[#filterDefFilter + 1] = "[word=WTS]"
	filterDefActive[#filterDefActive + 1] = false
...
--	print ("event=".. (event or "nil") .. " sender=" .. (sender or "nil"))
...
AddIgnore = function(name, noNote)

	--print("DEBUG: Info sent from Blizzard name="..(name or "nil") .. " note="..(noNote or "nil"))
...
DelIgnore = function(idxpos, isGIL)
	...
	--print ("DEBUG DelIgnore idx="..(idxpos or "nil"))
...
AddOrDelIgnore = function(name)
	...
	--print ("DEBUG AddOrDel called with: "..(name or "nil"))
