...
local function GilUnitMenu (dropdownMenu, which, unit, name, userData, ...)
	...
	if (which and (which == "PLAYER" or which == "RAID_PLAYER" or which == "PARTY" or which == "TARGET")) then
		...
		if name and name == RAID_TARGET_ICON then
			...
			info.func = function() AddOrDelIgnore(addServer(target)) GILUpdateUI(true) end
...
local function IgnoreButtonDoubleClick(self, ...)
	nameUI = addServer(GetIgnoreName(GetSelectedIgnore()))
...
local function IgnoreButtonClick(self, button, down)
	...
	local IgnoreRightClickMenu = {
		...
		{ text = GetIgnoreName(GetSelectedIgnore()), isTitle = true, notCheckable = true },
		...
		{ text = "", disabled = true, notCheckable = true },
		{ text = L["RCM_4"], notCheckable = true, func = function() DelIgnore(GetIgnoreName(GetSelectedIgnore())) end },
