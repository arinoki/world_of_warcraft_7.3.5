...
GIL synchronizes the ignore list across characters and servers and allows for an unlimited number of ignores, including NPC characters and entire servers!  GIL also does chat spam filtering and allows you to create your own custom filters!

Here is a list of some of the key features:

* Synchronize ignore list across characters and optionally servers
* Allow for more than 50 ignored players per character
* Ignoring one character ignores all characters on account *1
* Set ignore reasons and/or notes for each ignored player
* Optionally filter chat for spam and create your own custom chat filters!
* Optionally remove people from ignore list after a specific number of days
* Ignore non-player-characters (NPC) chat (both friendly and monsters)
* Ignore entire servers
...
See readme.txt for a description of the chat commands or type /gi in game for a reference.

*1 Note: Account based ignore can only work for the first 50 characters on each player's ignore list due to a Blizzard limitation, but GIL can handle thousands of ignores.  GIL by default will manage this by adding only same server characters to account-wide ignore to maximize the usefulness of the 50 account wide bans.
...
achivements, word matches, partial word matches, and any link.  The following
...
mean a spell, item, achievement, talent, battlepet, etc.
...
The talent tag works just the same as the other tags such as spell and item tags.
